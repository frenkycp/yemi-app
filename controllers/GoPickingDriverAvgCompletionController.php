...
class GoPickingDriverAvgCompletionController extends Controller
{
	...
	public function actionIndex()
	{
		...
		foreach ($tmp_driver_arr as $tmp_driver) {
			...
			$gojek_data_arr = GojekView01::find()
			->select([
				'JOB_COUNT' => 'SUM(JOB_COUNT)',
				'TOTAL_LT' => 'SUM(TOTAL_LT)',
			])
			->where([
				'ISSUE_PERIOD' => $period,
				'GOJEK_ID' => $tmp_driver->GOJEK_ID
			])
			->one();

			if ($gojek_data_arr->JOB_COUNT  > 0) {
				$tmp_avg = round($gojek_data_arr->TOTAL_LT / $gojek_data_arr->JOB_COUNT, 1);
			}
