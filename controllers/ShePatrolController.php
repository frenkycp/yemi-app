...
*/
class ShePatrolController extends Controller
{
	...
	public function actionSetArea()
	{
        ...
        $model->addRule(['area', 'team'], 'required');

        if ($model->load($_POST)) {
        	return $this->redirect(Url::to(['she-patrol/create', 'area' => $model->area, 'team' => $model->team]));
        }

        return $this->render('set-area', [
        	'model' => $model
        ]); 
	}

	public function actionCreate($area, $team)
	{
			...
			if ($model->load($_POST)) {
				...
				if ($model->save()) {
					...
					if ($model->upload_before_1) {
						$new_filename_b1 = $model->ID . '_SHE_PATROL_BEFORE_1.' . $model->upload_before_1->extension;
						$filePath_b1 = \Yii::getAlias("@app/web/uploads/SHE_PATROL/") . $new_filename_b1;
					...
					return $this->redirect(Url::to(['she-patrol/create', 'area' => $model->LOC_DESC, 'team' => $model->AUDITOR]));
				}
