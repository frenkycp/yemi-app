        ...
        if ($model->load($_GET)) {
        	...
        	if (!$tmp_setlist) {
	        	...
	        	->select([
	        		...
	        		'total_open' => 'SUM(CASE WHEN (status = \'C\' OR status = \'P\') AND hand_scan IS null THEN 1 ELSE 0 END)',
	        		'total_close' => 'SUM(CASE WHEN (status = \'C\' OR status = \'P\') AND hand_scan = \'Y\' THEN 1 ELSE 0 END)',
	        	])
	        	->where([
