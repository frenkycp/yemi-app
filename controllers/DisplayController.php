...
class DisplayController extends Controller
{
    ...
    public function actionMonthlyProgressSummaryEmail($type = 2)
    {
        $this->layout = 'clean';
        date_default_timezone_set('Asia/Jakarta');
        ...
        return $this->render('monthly-progress-summary-email', [
            'data' => $data,
            'model' => $model,
            'period_arr' => $period_arr,
            'top_minus' => $top_minus,
    ...
    public function actionShippingDisplayEmail($value='')
    {
        $this->layout = 'clean';
        date_default_timezone_set('Asia/Jakarta');
        //$today = date('Y-m-d');
        ...
        foreach ($data as $key => $value) {
            ...
            ->one();
            $total_container = SernoOutput::find()
            ->select([
                'cntr'
            ])
            ->where([
            ...
            $data_table[] = [
                'plan' => $tmp_so->qty,
                'actual' => $tmp_so->output,
                'balance' => $tmp_so->balance,
                'total_container' => $total_container,
            ];
        }
        ...
        foreach ($container as $key => $value) {
            ...
            $dataOpen[] = [
                ...
                'qty' => $value->balance,
                'url' => Url::to(['serno-output/index', 'index_type' => 1, 'etd' => $value->etd, 'dst' => $value->dst, 'back_order' => $value->back_order]),
            ];
            $dataClose[] = [
                'y' => $close_percentage > 0 ? $close_percentage : null,
                'qty' => $value->output,
                'url' => Url::to(['serno-output/index', 'index_type' => 2, 'etd' => $value->etd, 'dst' => $value->dst, 'back_order' => $value->back_order]),
            ];
            //$dataOpen[] = 50;
            //$dataClose[] = 50;
        ...
        return $this->render('shipping-display-email', [
    ...
    function actionVmsVsFloEmail($value='')
    {
        $this->layout = 'clean';
        date_default_timezone_set('Asia/Jakarta');
        $this_period = date('Ym');
        ...
        return $this->render('vms-vs-flo-email', [
            'model' => $model,
            'data' => $data,
    ...
    public function actionVmsVsFlo($value='')
    {
        $this->layout = 'clean';
        date_default_timezone_set('Asia/Jakarta');
        $this_period = date('Ym');
    ...
    public function actionIjazahProgress($code = '')
    {
        ...
        $tmp_gmc_arr = [];
        foreach ($tmp_ijazah_arr as $key => $value) {
            if (!isset($tmp_gmc_arr[$value->ITEM])) {
                $tmp_gmc_arr[$value->ITEM] = $value->ITEM_DESC;
            }
        }
        ...
        foreach ($tmp_ijazah_arr as $key => $value) {
            ...
            $actual_qty = $value->ACTUAL_QTY_ALLOC;
            $plan_amt = $actual_amt = $pct_amt = 0;
            if ($value->STD_PRICE != null && $value->STD_PRICE > 0) {
                $plan_amt = $plan_qty * $value->STD_PRICE;
                $actual_amt = $actual_qty * $value->STD_PRICE;
            }
            
            if ($plan_qty > 0) {
                $pct_qty = round(($actual_qty / $plan_qty) * 100, 1);
            }

            $tmp_data_arr[$value->ITEM][$period]['plan_qty'] = $plan_qty;
            $tmp_data_arr[$value->ITEM][$period]['actual_qty'] = $actual_qty;
            $tmp_data_arr[$value->ITEM][$period]['plan_amt'] = $plan_amt;
            $tmp_data_arr[$value->ITEM][$period]['actual_amt'] = $actual_amt;

            $plan_qty_period[$period] += $plan_qty;
            $actual_qty_period[$period] += $actual_qty;
            $plan_amt_period[$period] += $plan_amt;
            $actual_amt_period[$period] += $actual_amt;
        ...
        ksort($plan_qty_period);
        ksort($actual_qty_period);
        ksort($plan_amt_period);
        ksort($actual_amt_period);

        foreach ($tmp_gmc_arr as $gmc => $tmp_gmc) {
            foreach ($period_arr as $period) {
                if (!isset($tmp_data_arr[$gmc][$period])) {
                    $tmp_data_arr[$gmc][$period]['percentage'] = 0;
                    $tmp_data_arr[$gmc][$period]['plan_qty'] = 0;
                    $tmp_data_arr[$gmc][$period]['actual_qty'] = 0;
                }
            }
            ksort($tmp_data_arr[$gmc]);
        }
        ...
        return $this->render('ijazah-progress', [
            ...
            'actual_qty_period' => $actual_qty_period,
            'plan_amt_period' => $plan_amt_period,
            'actual_amt_period' => $actual_amt_period,
        ]);
    }
    ...
    public function actionFgsStockDetail($category)
    {
        $remark = '<div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
        ...
        /*if ($over_category == 1) {
            ...
            ->andWhere(['>=', 'tb_serno_output.etd', '2021-06-01'])
            ->andWhere(['pk_loading' => ''])
        ...
        ->joinWith('sernoMaster')
        ->select([
            'tb_serno_output.dst', 'tb_serno_output.gmc',
            'tb_serno_master.model', 'tb_serno_master.dest', 'tb_serno_master.color',
            'stock_qty' => 'SUM(output - loading)'
        ])
        ->where(['etd' => $category])
        ->andWhere(['<>', 'output', 'loading'])
        ->groupBy('tb_serno_output.dst', 'tb_serno_input.gmc')
        ->orderBy('tb_serno_output.dst', 'tb_serno_input.gmc')
        ->all();
        ...
        foreach ($data_arr as $key => $value) {
            $remark .= '<tr style="font-size: 14px;">
                ...
                <td class="text-center">' . $value->stock_qty . '</td>
            </tr>';
            $no++;
        }
    ...
    public function actionFgsStock($value='')
    {
        ...
        $tmp_input_arr = SernoOutput::find()
        ->select([
            'etd', 'dst', 'stock_qty' => 'SUM(output - loading)'
        ])
        ->where('etd >= CURDATE()')
        ->andWhere(['<>', 'output', 'loading'])
        ->groupBy('etd, dst')
        ...
        foreach ($tmp_input_arr as $key => $value) {
            ...
            $tmp_data[$tmp_title] += $value->stock_qty;

            if (!isset($tmp_data3[$tmp_title][$value->dst])) {
                $tmp_data3[$tmp_title][$value->dst] = 0;
            }
            $tmp_data3[$tmp_title][$value->dst] += $value->stock_qty;
        }
        ...
        foreach ($tmp_data as $key => $value) {
            ...
            $tmp_data2[] = [
                'y' => $value,
                'url' => Url::to(['fgs-stock-detail', 'category' => $key]),
            ];
        }
    ...
    public function actionMonthlyOvertimeBySectionNew()
    {
        $this->layout = 'clean';
        $model = new \yii\base\DynamicModel([
            'section', 'from_date', 'to_date'
        ...
        return $this->render('monthly-overtime-by-section-new', [
            'data' => $data,
            'tmp_data' => $tmp_data,
            'model' => $model,
            'section' => $section,
